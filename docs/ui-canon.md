...
- `Collapse` handles menu and FAQ visibility. `disclosure` avoids Tailwind's unrelated visibility utility. FAQ callers own native question buttons, `aria-expanded`, `aria-controls`, and named answer regions. Reduced-motion preferences remove animation.
